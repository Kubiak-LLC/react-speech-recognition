...
var RecognitionManager = /*#__PURE__*/function () {
  ...
  _createClass(RecognitionManager, [{
    ...
    key: "emitResultsChange",
    value: function emitResultsChange(fullResults) {
      var _this3 = this;

      Object.keys(this.subscribers).forEach(function (id) {
        var onResultsChange = _this3.subscribers[id].onResultsChange;
    ...
    value: function updateTranscript(_ref) {
      ...
      for (var i = currentIndex; i < results.length; ++i) {
        console.log("loop ".concat(i, ": \n").concat(JSON.stringify(results[i])));

        if (results[i].isFinal && (!(0, _isAndroid["default"])() || results[i][0].confidence > 0)) {
          console.log("updateFinalTranscript ".concat(i, ": \n").concat(JSON.stringify(results[i][0])));
          console.log("updateFinalTranscript ".concat(i, ": \n").concat(JSON.stringify(results[i][0].transcript)));
          this.updateFinalTranscript(results[i][0].transcript);
        } else {
          console.log("interimTranscript ".concat(i, ": \n").concat(JSON.stringify(results[i][0])));
          console.log("interimTranscript ".concat(i, ": \n").concat(JSON.stringify(results[i][0].transcript)));
